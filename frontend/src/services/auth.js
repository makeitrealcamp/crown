...
class AuthService extends Store {
  ...
  async _doLoadUser(token) {
    ...
    this.setState({
      username, email, admin, token, loaded: true,
    })
  }
  ...
  async signup(user) {
    ...
    this.setState({
      firstName: user.firstName, lastName: user.last_name, email: user.email, token, loaded: true,
    })
  }
  ...
  async login(emailArg, password) {
    ...
    const {
      firstName, lastName, email, admin, token,
    } = response.data
    localStorage.setItem('token', token)
    this.setState({
      firstName, lastName, email, admin, token, loaded: true,
    })
  }
